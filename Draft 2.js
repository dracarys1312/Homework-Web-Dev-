...
function sendAllPayslips(){
  ...
  var button = uiMsgPrompt.getSelectedButton();
  var messageNoName = uiMsgPrompt.getResponseText();
...
function onEdit() {
    ...
    for (var r = 0; r < codeValues.length; r++){
        var row = codeValues[r];
        var code = row[0];
        Logger.log(toViewCell.getValue());
        if(toViewCell.getValue() == code){
            var instrInfo = loadInstrInfo(code);
            var salary = loadSalary(code);
...
function printSalary(salary, sheet){
 ...
 function sendThisPaySlips(){
   ...
   var button = uiEmailPrompt.getSelectedButton();
